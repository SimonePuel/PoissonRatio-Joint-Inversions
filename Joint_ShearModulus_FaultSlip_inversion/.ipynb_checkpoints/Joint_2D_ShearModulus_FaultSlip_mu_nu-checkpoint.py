"""
This code solves a joint adjoint-based deterministic inversion 
of synthetic 2D surface geodetic data (e.g. GNSS sensors or InSAR) 
for the variation in the heterogeneous material structure and 
...
def save_results(mesh, Vh, xf, yf, mu_true, m_mu, s_true, m_s, d_obs, d_cal, u):
    # PARAMETER (mu_true, m_mu)
    filename = 'm_mu'
    function_names = ['true', 'inferred']
    function_list = [mu_true, m_mu]
    FunctionsIO.write(mesh.mpi_comm(), savepath + filename, function_names, function_list)

    # xf, yf
    fault = np.array([ xf, yf ]) 
    tosave = pd.DataFrame(fault.T, columns=['xf', 'yf'])
    filename = 'fault_geometry.txt'
    tosave.to_csv(savepath + filename, sep=' ', index=False)
    ...
    with dl.XDMFFile(mesh.mpi_comm(), savepath + filename + ".xdmf") as fid:
            fid.write_checkpoint(u_save, 'u')
...
###---------------------###
# Define the expression for shear modulus
def mu_expression(m):
...
mean_TV = dl.Expression( ( '0.', '0.' ), degree=0 ) 
...
# Solve the inverse problem
( mesh, Vh, targets, xf, yf, idx_f, u, mu_true, m_mu, s_true, m_s, d_obs, d_cal ) = solveJointInversion(mesh, boundaries, k, targets, noise_std_dev, mtrue_mu_expr, mtrue_s_expr, mean_TV, m0_mu_s_expr, gamma_val_TV, gamma_val_H1_mu, gamma_val_H1_s, delta_val_L2, eps, generate_synthetics=generate_synthetics, verbose=True)
# Save results
save_results( mesh, Vh, xf, yf, mu_true, m_mu, s_true, m_s, d_obs, d_cal )
