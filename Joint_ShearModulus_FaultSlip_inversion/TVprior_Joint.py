...
# Create a class for the Total Variation (TV) regularization
class vectorTVH1L2handler:
    ...
    """
    def __init__(self, gamma_val_TV, gamma_val_H1_mu, gamma_val_H1_s, delta_val_L2, eps=1.e-4):
        self.gamma_TV    = dl.Constant( gamma_val_TV )
        self.gamma_H1_mu = dl.Constant( gamma_val_H1_mu )
        self.gamma_H1_s  = dl.Constant( gamma_val_H1_s )
    ...
    def __call__(self, m):
        m_mu, m_s = dl.split(m)
        return ( self.gamma_TV * self._w_TV(m_mu) \
                 + self.gamma_H1_mu * dl.Constant(.5) * dl.inner( dl.grad(m_mu), dl.grad(m_mu) ) \
                 + self.gamma_H1_s  * dl.Constant(.5) * dl.inner( dl.grad(m_s), dl.grad(m_s) ) \
                 + self.delta_L2    * dl.Constant(.5) * ( dl.inner(m_mu, m_mu) + dl.inner(m_s, m_s) ) \
                )*dl.dx

    def gn_hessian(self, m, mtrial, mtest):
        m_mu, m_s = dl.split(m)
        m_mu_trial, m_s_trial = dl.split(mtrial)
        m_mu_test, m_s_test = dl.split(mtest)
        return self.gamma_TV * dl.Constant(1.)/self._w_TV(m_mu) * ( dl.inner( dl.grad(m_mu_trial), dl.grad(m_mu_test)) )*dl.dx \
             + self.gamma_H1_mu * dl.inner( dl.grad(m_mu_trial), dl.grad(m_mu_test) )*dl.dx \
             + self.gamma_H1_s * dl.inner( dl.grad(m_s_trial), dl.grad(m_s_test) )*dl.dx \
             + self.delta_L2 * ( dl.inner(m_mu_trial, m_mu_test) + dl.inner(m_s_trial, m_s_test) )*dl.dx
...
class NonGaussian_Prior( hp.modeling.prior._Prior ):
    ...
    gamma_val_TV, gamma_val_H1_mu, gamma_val_H1_s, delta_val_L2: Regularization parameters (must be floats)

    prior_handler = vectorTVH1L2handler(gamma_val_TV, gamma_val_H1_mu, gamma_val_H1_s, delta_val_L2)
